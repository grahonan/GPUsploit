...
#include<stdio.h>
using namespace std;
//two thread in same block and considering local memory 
//when both fp and buf must allocate in shared memory， it cause overflow
#define BUF_LEN 16
#define N 2
...
__device__  unsigned long __noinline__ unsafe(unsigned int *input, int len)
{
	...
	fp[3] = dummy4;
	fp[4] = dummy5;
	if (threadIdx.x == 0)//to skip other thread rewrite
		fp[5] = dummy6;
	fp[6] = dummy7;
	fp[7] = dummy8;
...
__global__ void test_kernel(unsigned long *hashes, unsigned int *input, int len, int admin)
{
	...
	else
	{
		if (idx == 0)
			...
			my_hash = unsafe(input + (len*idx), 16);
		}
